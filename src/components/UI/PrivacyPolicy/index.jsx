...
const PrivacyPolicy = () => {
  ...
  const { data, isLoading } = useInfoQuery({ data: { privacy: true } });
  const privacy = data && data[0];

  const { data: titles } = useStateTitlesQuery({
    id: privacy && privacy.guid,
    data: { info_id: [privacy && privacy.guid] },
    queryParams: { enabled: !!(privacy && privacy.guid) },
  });
  ...
  return (
    <main className={cls.main}>
      <Container>
        ...
        <h1 className={cls.title}>Политика конфиденциальности</h1>
        {isLoading ? (
