...
import cls from "./Rekvizit.module.scss";
...
const Rekvizit = () => {
  ...
  const { data, isLoading } = useInfoQuery({ data: { rekvizit: true } });
  const rekvizit = data && data[0];

  const { data: titles } = useStateTitlesQuery({
    id: rekvizit && rekvizit.guid,
    data: { info_id: [rekvizit && rekvizit.guid] },
    queryParams: { enabled: !!(rekvizit && rekvizit.guid) },
  });
  ...
  return (
    <main className={cls.main}>
      <Container>
        ...
        <h1 className={cls.title}>Реквизиты PARAGRAF Navoiy</h1>
        {isLoading ? (
